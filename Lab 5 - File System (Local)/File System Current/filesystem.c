...
void* testThread(){
    ...
    printf("Write to file \"Test 2\": %d\n", fs_write(file3, &read, sizeof(read)));

    char read2[200] = "\0";
    printf("Read from file \"Test 2\": %d\n", fs_read(file3, read2, sizeof(read2)));
    printf("Output from read: %s\n", read2);
...
int fs_delete(char *name){
    ...
    memset(logic.root[root].name, 0, sizeof(logic.root[root].name));
    return 0;
}
...
int fs_truncate(int filedes, off_t length) {
    ...
    logic.root[root].fileSize = length;
    fileDesc.fds[1][filedes] = length;
